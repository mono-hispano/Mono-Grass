...
<body>
    ...
    <div id="global_domain_bar">
        <div>
            <a href="/"><strong>MONO-HISPANO</strong>.ORG</a>
        </div>
    </div>
    ...
    <div id="header" class="container_12">
        <div id="logo" class="grid_3">
            <h1><a title="<?php _e( 'Go to home page', 'grass' ); ?>" href="<?php echo home_url('/'); ?>"><img src="<?php bloginfo('stylesheet_directory'); ?>/images/mono-hispano-logo.png" alt="<?php echo _e('MONO HISPANO', 'grass');?>" /></a></h1>
        </div>
        <div id="top_bar" class="grid_9">
            ...
            <div class="right">
                <form role="search" method="get" id="searchform" action="<?php echo home_url('/'); ?>" >
                    <div>
                        <label class="hidden" for="s"><?php _e( 'Buscar', 'grass' ); ?>: </label><input type="text" value="<?php if(isset($_GET['s'])) { echo htmlspecialchars(stripslashes(strip_tags($_GET['s']))); } ?>" name="s" id="s" placeholder="<?php _e( 'Buscar', 'grass' ); ?>" />
                    </div>
                </form>
            </div>
        </div>
